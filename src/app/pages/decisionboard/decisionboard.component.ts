...
@Component({
  selector: 'app-decisionboard',
  templateUrl: './decisionboard.component.html',
  styleUrls: ['./decisionboard.component.css'],
})
export class DecisionboardComponent implements OnInit {
  ...
  ChevkCorrect(): void {
    ...
    this.router.navigate(['Examhub/CorrectAnswer'], {
      queryParams: {
  ...
  nextRound(): void {
    ...
    if (data <= this.data[4]) {
      ...
      this.router.navigate(['Examhub/Alquizhub'], {
        queryParams: {
      ...
      Swal.fire('Level completed!', '!', 'success').then((value) => {
        this.router.navigate(['Examhub/ALevel']);
      });
    }
  }
  ...
  setLeaderboard(): void {
    ...
    this.service.setMonthlyLeaderboardAl(score).subscribe((res) => {});
  }
  ...
  setStatics(): void {
    ...
    this.service.setStaticsAL(Statics).subscribe(
      (res) => {},
      (err) => {
