...
@Component({
  selector: 'app-desisionboardsix',
  templateUrl: './desisionboardsix.component.html',
  styleUrls: ['./desisionboardsix.component.css'],
})
export class DesisionboardsixComponent implements OnInit {
  ...
  ChevkCorrect(): void {
    ...
    this.router.navigate(['Examhub/610CorrectAnswer'], {
      queryParams: {
  ...
  setLeaderboard(): void {
    ...
    this.service.setMonthlyLeaderboardSix(score).subscribe((res) => {});
  }
  setStatics(): void {
    ...
    this.service.setStaticsSix(Statics).subscribe(
      (res) => {},
      (err) => {
  ...
  nextRound(): void {
    ...
    if (data <= this.data[4]) {
      ...
      this.router.navigate(['Examhub/610Quizhub'], {
        queryParams: {
