...
@Component({
  selector: 'app-sixquizhub',
  templateUrl: './sixquizhub.component.html',
  styleUrls: ['./sixquizhub.component.css'],
})
export class SixquizhubComponent implements OnInit {
  data: any;
  subQuiz: any;
  quiz: any;
  le: any;
  ...
  ngOnInit(): void {
    ...
    $.default('#le').addClass('id' + num); // Add Class
    ...
    this.Serviceservice.getquestionsix(ques).subscribe(
      (event) => {
        this.subQuiz = event;
    ...
    this.Serviceservice.levelbyQuestionsix(levlQuiz).subscribe(
      (event) => {
        this.question = event;
  ...
  getcoind(): void {
    ...
    this.Serviceservice.getdata(id).subscribe((res) => {
      const coins = res;
  ...
  getnumber(): void {
    ...
    if (this.count < this.leng - 1) {
      ...
      datanew.append('user_id', this.data[2][1]);
      ...
      setTimeout(() => {
        ...
        if (this.number >= 3) {
          this.router.navigate(['Examhub/610Decisionboard'], {
            queryParams: { data: JSON.stringify(valuenumber) },
          });
        } else {
          this.router.navigate(['Examhub/610Decisionboardtry'], {
            queryParams: { data: JSON.stringify(valuenumber) },
          });
        }
      }, 3000);
