...
@Component({
  selector: 'app-alquizhub',
  templateUrl: './alquizhub.component.html',
  styleUrls: ['./alquizhub.component.css'],
})
export class AlquizhubComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.Serviceservice.getquestion(ques).subscribe(
      (event) => {
        this.subQuiz = event;
    ...
    this.Serviceservice.levelbyQuestion(levlQuiz).subscribe(
      (event) => {
        this.question = event;
  ...
  getnumber(): void {
    if (this.count < this.leng - 1) {
      ...
      setTimeout(() => {
        ...
        if (this.number >= 3) {
          this.router.navigate(['Examhub/Decisionboard'], {
            queryParams: {
              data: JSON.stringify(valuenumber),
            },
          });
        } else {
          this.router.navigate(['Examhub/Decisionboardtry'], {
            queryParams: {
              data: JSON.stringify(valuenumber),
            },
          });
  ...
  removeprecentage(): void {
    ...
    const hidden5 = document.getElementById('E');
    hidden5.classList.remove('circle2');
  }
  getDataUser(): void {
